...
% load data
test_case_idx = 1;
[vGraph, pointsWithData, goodAxis, polytopes, start, finish, addNudge] = fcn_INTERNAL_loadExampleData_fcn_VGraph_plotVGraph(test_case_idx);
...
h_plot = fcn_VGraph_plotVGraph(vGraph, pointsWithData, 'g-');
axis(goodAxis);
...
function [vGraph, pointsWithData_with_startfinish, goodAxis, polytopes, startPointData, finishPointData, addNudge] = ...
...
startPointData = [startXY size(pointsWithData,1)+1 -1 1]; 
finishPointData = [finishXY size(pointsWithData,1)+2 -1 1]; 
pointsWithData_with_startfinish = [pointsWithData; startPointData; finishPointData];
