...
% 
% As: script_visibility_graph_modification
...
raw_polytopes = fcn_MapGen_generatePolysFromSeedGeneratorNames('haltonset', [1+Halton_seed 11+Halton_seed],[], ([]), (-1));
...
polytopes = fcn_MapGen_polytopesShrinkEvenly(trim_polytopes, 0.025, (-1));
...
idx_of_polytope_for_removal = 4;
polytope_for_removal = polytopes(idx_of_polytope_for_removal);
if flag_do_plot
    figNum = 99; % figure to plot on
    fcn_MapGen_plotPolytopes(polytope_for_removal,'r',[1 0 0 0 0.5],figNum);

    % line_spec = 'r--'; % edge line plotting
    % fcn_plot_polytopes(polytope_for_removal,figNum,line_spec,line_width,axes_limits,axis_style);
    title('original map with polytope for removal highlighed')
end

modify_vgraph_timer = tic;
[vgraph_new, all_pts_new, start_new, finish_new, new_polytopes] = fcn_visibility_graph_remove_obstacle(vgraph, pointsWithData, [], [], polytopes, idx_of_polytope_for_removal);
toc(modify_vgraph_timer)

if flag_do_plot
    figNum = 999; % figure to plot on
    % line_spec = 'b-'; % edge line plotting
    % line_width = 2; % linewidth of the edge
    % axes_limits = [0 1 0 1]; % x and y axes limits
    % axis_style = 'square'; % plot axes style
    % fcn_plot_polytopes(new_polytopes,figNum,line_spec,line_width,axes_limits,axis_style);
    fcn_MapGen_plotPolytopes(new_polytopes,'b',[1 0 0 0 0.5],figNum);
...
if flag_do_plot
    figNum = 9999; % figure to plot on
    % line_spec = 'b-'; % edge line plotting
    % line_width = 2; % linewidth of the edge
    % axes_limits = [0 1.5 0 1.5]; % x and y axes limits
    % axis_style = 'square'; % plot axes style
    % fcn_plot_polytopes(new_polytopes2,figNum,line_spec,line_width,axes_limits,axis_style);
    fcn_MapGen_plotPolytopes(new_polytopes2,'b',[1 0 0 0 0.5],figNum);
    fcn_MapGen_plotPolytopes(polytope_to_add,'r',[1 0 0 0 0.5],figNum);
    
    % line_spec = 'r--'; % edge line plotting
    % fcn_plot_polytopes(polytope_to_add,figNum,line_spec,line_width,axes_limits,axis_style);
...
if flag_do_plot
    figNum = 99999; % figure to plot on
    % line_spec = 'b-'; % edge line plotting
    % line_width = 2; % linewidth of the edge
    % axes_limits = [0 1.5 0 1.5]; % x and y axes limits
    % axis_style = 'square'; % plot axes style

    fcn_MapGen_plotPolytopes(new_polytopes3,'r',[1 0 0 0 0.5],figNum);
    fcn_MapGen_plotPolytopes(polytope_to_add2,'r',[1 0 0 0 0.5],figNum);

    % fcn_plot_polytopes(new_polytopes3,figNum,line_spec,line_width,axes_limits,axis_style);
    % line_spec = 'r--'; % edge line plotting
    % fcn_plot_polytopes(polytope_to_add2,figNum,line_spec,line_width,axes_limits,axis_style);
    title('original map with polytope for addition highlighed')
    hold on
    box on
    xlabel('x [km]')
