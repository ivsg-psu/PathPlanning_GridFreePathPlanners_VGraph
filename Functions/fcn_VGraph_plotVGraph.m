function h_plot = fcn_VGraph_plotVGraph(vGraph, pointsWithData,  styleString, varargin)
...
if 0==flag_max_speed
    if flag_check_inputs
        ...
        % Check that pointsWithData is same size as visibility
        assert(length(pointsWithData(:,1))==length(vGraph(:,1)));
...
for ith_fromIndex = fromRange
    ...
    for jth_toIndex = toRange
        if vGraph(ith_fromIndex,jth_toIndex) == 1
            thisFromPointsToPlot = [thisFromPointsToPlot; [pointsWithData(ith_fromIndex,1:2); pointsWithData(jth_toIndex,1:2); nan(1,2)]]; %#ok<AGROW>
        end
    end
