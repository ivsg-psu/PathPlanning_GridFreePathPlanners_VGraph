...
%% DEMO case: check if single point is inside a single polytope
figNum = 10001;
titleString = sprintf('DEMO case: check if single point is inside a single polytope');
...
clear polytopes
polytopes = struct;
polytopes.vertices = [verticesXY; verticesXY(1,:)];
polytopes.xv = xv;
polytopes.yv = yv;
...
%% DEMO case: check if many points are inside several polytopes
figNum = 10002;
titleString = sprintf('DEMO case: check if many points are inside several polytopes');
fprintf(1,'Figure %.0f: %s\n',figNum, titleString);
figure(figNum); clf;
...
%      try
%          fcn_VGraph_polytopePointsInPolytopes(A,B,polytopes)
%      catch
